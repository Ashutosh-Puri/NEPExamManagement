<div class="m-2 overflow-hidden rounded border bg-white shadow dark:border-primary-darker dark:bg-darker">
  <div class="bg-primary px-2 py-2 font-semibold text-white dark:text-light">
    Create Class Wise Exam Time Table <x-spinner />
  </div>
        ...
        <x-table.tbody>
          @forelse ($subjects as $subject)
            <x-table.tr wire:key="{{ $subject->id }}">
              ...
              <x-table.td>
                <x-table.text-scroll class="w-full"> {{ $subject->subject_code }}
                  {{ $subject->subject_name }} </x-table.text-scroll>
              </x-table.td>
              <x-table.td>
                <x-text-input id="examdates.{{ $subject->id }}" type="date" wire:model="examdates.{{ $subject->id }}" name="examdates.{{ $subject->id }}" class="w-full min-w-40 mt-1" />
                @error('examdates.{$subject->id}')
